...
/* Bridge into the LVGL theme layer for the real button-driven
 * screen-switch/press-handling calls above (screenShow()/screenOnPress()
 * are still genuinely called from mainscreen.c's real button dispatch -
 * only screen.c's own renderer, which used to consume them, is gone).
 * ugui_shim.c's screenShow() stores the most recently requested Screen*
 * here as a one-shot request (dashboard_theme_tick() clears it once
 * handled, so a stale value can't re-trigger anything on a later tick);
 * screenOnPress() forwards to whichever handler is currently registered
 * here, so the active LVGL screen gets first refusal on every button
 * event exactly like a real Screen.onPress would - set/cleared by the
 * LVGL screen itself (theme_osf_modern.c's config screen), not by any
 * theme-agnostic code. */
extern Screen *g_lvgl_requested_screen;
extern bool (*g_lvgl_screen_on_press)(buttons_events_t events);
...
#elif defined(DISPLAY_850C) || defined(DISPLAY_850C_2021)
  #define SCREENFN_FORCE_LABELS false
