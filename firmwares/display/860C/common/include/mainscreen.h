...
/* Same lookup warnings() uses to render "e: <text>" on the warning banner
 * - shared here so the config screen's error-history fields (History
 * Errors -> Last error 1-4) can show the same text instead of a raw index. */
