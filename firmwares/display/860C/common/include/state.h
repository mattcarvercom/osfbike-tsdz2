...
//#ifndef SW102
  ...
  uint8_t ui8_active_theme_index; // index into g_available_themes[] (dashboard_theme.h)

  // Which live metric each of the main screen's two mini-cards shows (see
  // MiniCardOption in theme_osf_modern.c for the option list/ordering) and
  // which source feeds the mini-graph (currently only one real option,
  // "Speed/Avg Speed" - the field exists now so a future second option
  // doesn't need another EEPROM version bump).
  uint8_t ui8_mini_card_1_field;
  uint8_t ui8_mini_card_2_field;
  uint8_t ui8_mini_graph_field;

  // Index into POWER_BAR_SCALE[] (theme_osf_modern.c) - multiplies the
  // human/motor power bars' fill percentage (display only, never the
  // watts number shown below them) so a lower-power rider/motor can still
  // visually reach the top of the bar instead of it looking permanently
  // half-empty. Default index 0 = 1x (no scaling).
  uint8_t ui8_human_power_bar_scale;
  uint8_t ui8_motor_power_bar_scale;

  // Header bar's battery readout (Configuration -> Display -> "Battery
  // field") - 0=percentage (default, current behavior), 1=disabled (hides
  // the bar+label entirely, same "leaves a gap" convention as Clock field's
  // own disable), 2=battery voltage (bar stays, label switches to volts).
  uint8_t ui8_battery_field_enable;

  // Temperature icon/color feedback: how many degrees before
  // ui8_motor_temperature_min_limit_value the icon turns yellow (signed,
  // negative by default), plus the icon/value visibility toggles.
  // Display-local only, never transmitted to the motor. Named ui8_ to match
  // this codebase's "byte-sized field" convention even though the offset is
  // int8_t.
  int8_t ui8_temp_min_warn_offset;
  uint8_t ui8_display_temp_icon_enabled;
  uint8_t ui8_display_temp_value_enabled;
...
/* Real logic (state.c) - see its own doc comment. Exposed here so
 * wasm-display-sim/sim_glue.c can call it directly every tick, since the
 * sim never drives the real 100ms UART ISR path that normally reaches it. */
