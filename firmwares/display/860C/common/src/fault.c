...
/* faultCode/addrCode/infoCode themselves are defined in theme_osf_modern.c,
 * not here - that file is compiled into both the real firmware and the
 * WASM display sim, while this file (fault.c) is real-firmware-only
 * (wasm-display-sim/build.sh excludes it - see its own comment there for
 * why). build_fault_screen() (theme_osf_modern.c) reads these same three
 * Fields' text directly, so they need one shared definition reachable from
 * both builds; this file only writes into them via fieldPrintf() below. */
extern Field faultCode, addrCode, infoCode;
  ...
  case NRF_FAULT_ID_SDK_ERROR:
	...
	while (1) {
      ...
      sd_nvic_SystemReset();
		...
		/* Control never returns to main()'s own while(1) loop once we're
		 * parked here, so main.c's lv_timer_handler() call is never reached
		 * again either - without this, panicScreenShow() above would have
		 * built and loaded the fault screen for nothing, since LVGL never
		 * gets a chance to actually flush it to the panel. lv_tick_inc()
		 * keeps running independently (860C_850C/src/timers.c's real 1ms
		 * timer ISR, still firing here - this is an ordinary busy-wait, not
		 * fault-ISR context, for every path into this function except the
		 * hardware HardFault trap itself). */
		lv_timer_handler();
	}
}
