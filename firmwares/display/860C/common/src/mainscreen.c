...
/* Exposes ui8_set_riding_mode (static above) read-only, for the LVGL theme
 * (theme_osf_modern.c) to show that UP/DOWN is currently cycling assist
 * mode (Power/Torque/Cadence/eMTB/Hybrid, ui_vars.ui8_riding_mode) rather
 * than assist level. "riding_mode" here is this codebase's established
 * stock/vendor name for that field - NOT the same concept as Configuration
 * -> "Riding mode" (ui_vars.ui8_street_mode_enabled, configscreen.c), which
 * is street-vs-off-road. See appwide_onpress()'s SCREENCLICK_NEXT_SCREEN
 * branch above for where this flag actually gets toggled (PWR while assist
 * level is 0). */
bool mainScreenIsSelectingAssistMode(void) {
	return ui8_set_riding_mode;
}
...
#ifndef SW102
static const char *motorErrors[] = { "None", "Motor not init", "Torque Fault", "Cadence Fault", "Motor Blocked", "Throttle Fault", "Fatal Error", "Overcurrent", "Speed Fault", "Undervoltage"};
static const char *serviceMode[] = { "0", "chain", "brakes", "shocks", " "};
#else
static const char *motorErrors[] = { "None", "Mot no ini", "Torq Fault", "CadenFault", "MotBlocked", "Thro Fault", "FatalError", "Overcurren", "SpeedFault", "Undervolt"};
#endif

/* index is a raw byte pulled from ui_vars.ui8_last_error[] (configscreen.c's
 * History Errors fields) - out-of-range values (never written, or a slot
 * that's never recorded an error yet) fall back to motorErrors[0] rather
 * than indexing off the end of the table. */
const char *motor_error_text(uint8_t index) {
	if (index >= (sizeof(motorErrors) / sizeof(motorErrors[0])))
		index = 0;
	return motorErrors[index];
}
...
static bool appwide_onpress(buttons_events_t events)
{
  ...
  /* SCREENCLICK_START_CUSTOMIZING (long-press M on 860C, UPDOWN_LONG_CLICK
   * on 850C) used to be gated on ui_vars.ui8_assist_level so it wouldn't
   * collide with screen.c's real on-screen field-customization feature
   * (long-press M there picks a new value for a FIELD_CUSTOMIZABLE slot,
   * see screen.c's g_curCustomizingField/SCREENCLICK_STOP_CUSTOMIZING) -
   * that feature only made sense to reserve the combo for while actively
   * riding. screen.c's own µGUI renderer isn't compiled into this LVGL
   * build at all (ugui_shim.c replaces it - see its own doc comment), so
   * there's nothing left for this combo to collide with at PAS 0; the gate
   * just meant long-press-M silently did nothing at PAS 0, the exact bug
   * reported 2026-08-20. Now unconditional, matching
   * SCREENCLICK_ENTER_CONFIGURATIONS's own always-on behavior. */
