...
// last assist level seen while the walk assist button was not pressed; some displays
// (DZ40/VLCD5 family) report a decremented assist level while their walk assist button
// (shared with level-down) is held, so the live level can't be trusted during walk assist
static uint8_t ui8_assist_level_before_walk_button = ECO;
// set while the walk-assist button is held at an overridden level (any of
// ECO/TOUR/SPORT/TURBO individually, per CRUISE_OVERRIDE_ACTIVE_LEVEL) -
// riding mode has been temporarily swapped to CRUISE_MODE and needs
// restoring to ui8_riding_mode_temp on release
static uint8_t ui8_cruise_override_flag = 0;
...
// ---------------------------------------------------------------------------
// CRC16 860C/850C protocol state.
// Reuses ui8_rx_buffer/ui8_tx_buffer/ui8_received_package_flag/ui8_state_machine/
// ui8_byte_received above, sized via UART_RX_BUFFER_LEN/UART_TX_BUFFER_LEN in
// main.h (88/29 when this flag is on).
// ---------------------------------------------------------------------------
...
// motor init state machine (net-new)
...
// motor error bitmask reported to the display (M_ERROR_* in main.h).
// Starts at "Motor not init" because configurations are empty at startup.
static uint8_t ui8_m_system_state = M_ERROR_NOT_INIT;

// runtime values received each PERIODIC tick
...
static uint8_t ui8_target_battery_max_power_div25 = 20; // 500W default
static uint8_t ui8_target_battery_max_power_div25_temp = 0;

// runtime values received in the CONFIGURATIONS frame. Seeded from config.h
// defaults so the motor is safe before the first frame arrives.
static uint8_t ui8_smooth_start_enabled = SMOOTH_START_ENABLED;
static uint8_t ui8_smooth_start_counter_set = 0;
static uint8_t ui8_startup_boost_cadence_step = STARTUP_BOOST_CADENCE_STEP;
static uint8_t ui8_coaster_brake_torque_threshold = COASTER_BRAKE_TORQUE_THRESHOLD;
static uint8_t ui8_field_weakening_feature_enabled = FIELD_WEAKENING_ENABLED;
static uint8_t ui8_torque_modes_based_on_power = TORQUE_MODES_BASED_ON_POWER;
static uint8_t ui8_extended_boost_enabled = 0;
static uint8_t ui8_extended_boost_multiplier = 0;
static uint8_t ui8_extended_boost_threshold = 0;
static uint8_t ui8_extended_boost_ramp_down = 0;
static uint16_t ui16_power_based_reference_voltage_x10 = POWER_BASED_REFERENCE_VOLTAGE_X10;
#endif
...
static uint8_t m_system_state_get(void);
#endif
...
static void check_battery_sag_indicator(void);
...
void ebike_app_init(void)
{
	...
	if (m_configuration_variables.ui8_torque_sensor_estimated) {
		...
		ui8_pedal_torque_per_10_bit_ADC_step_x100_array[TORQUE_STEP_DEFAULT] = m_configuration_variables.ui8_pedal_torque_per_10_bit_ADC_step_est_x100;
	}
	// set pedal torque per 10_bit ADC step x100 advanced (calibrated)
	if (ui8_torque_sensor_calibrated) {
		...
		ui8_pedal_torque_per_10_bit_ADC_step_x100_array[TORQUE_STEP_ADVANCED] = m_configuration_variables.ui8_pedal_torque_per_10_bit_ADC_step_est_x100;
	}
	...
	ui32_adc_battery_power_max_x10_array[OFFROAD_MODE] = (uint32_t)((uint32_t)TARGET_MAX_BATTERY_POWER * 1000U)
		/ BATTERY_CURRENT_PER_10_BIT_ADC_STEP_X100;
...
#ifdef TIME_DEBUG
static uint16_t ebike_calc_us[4];
static uint16_t ebike_calc_us_max[4];
#endif
...
static void ebike_control_motor(void)
{
...
#if ENABLE_860C_LVGL_UART
	// virtual throttle from the display (byte[11] of the PERIODIC frame). The
	// 860C protocol drives throttle from the display rather than the compile-time
	// OPTIONAL_ADC_FUNCTION setting, so this is handled separately.
	if ((ui8_throttle_feature_enabled) && (ui8_throttle_virtual)) {
...
#if ENABLE_860C_LVGL_UART
	// motor init delay: after a CONFIGURATIONS frame is applied, wait 40 ticks
	// before clearing the "not initialized" state and re-enabling the motor.
	switch (ui8_m_motor_init_state) {
	  case MOTOR_INIT_STATE_INIT_START_DELAY:
		...
		// fallthrough - no break to execute next code
	  ...
	  case MOTOR_INIT_STATE_INIT_WAIT_DELAY:
		if (m_ui8_got_configurations_timer > 0) {
			...
			ui8_m_system_state &= ~M_ERROR_NOT_INIT;
		}
		break;
...
#if ENABLE_860C_LVGL_UART
	  || (ui8_m_system_state & (M_ERROR_NOT_INIT | M_ERROR_FATAL))
#endif
...
static void apply_rev_matching(void){
	...
	if ((ui8_riding_torque_mode == 1U) && (ui8_pedal_cadence_RPM > 0U)) {
		...
		uint8_t cadence_motor_pedal_sync_delta = (cadence_from_motor_RPM < ui8_pedal_cadence_RPM) ? (ui8_pedal_cadence_RPM - cadence_from_motor_RPM) : 0;
...
#define CRUISE_PID_OUTPUT_LIMIT					1000
...
#endif
	{
		...
		if (ui8_cruise_PID_initialize) {
			...
			// same decremented-level display quirk noted at ui8_riding_mode_parameter's
			// assignment (uart_receive_package()) applies here: while the walk-assist
			// button drives the override, use the level from before it was pressed
			ui16_wheel_speed_target_x10 = (uint16_t) (ui8_riding_mode_parameter_array[CRUISE_MODE - 1]
				[ui8_cruise_override_flag ? ui8_assist_level_before_walk_button : ui8_assist_level] * 10U);
#if ASSIST_LEVEL_5_MODE
			if (ui8_assist_level_5_flag) {
		...
		if (ui8_cruise_override_flag) {
			// Override engaged via the walk-assist button at real riding speed,
			// usually under load (climbing/headwind) - use the same speed/cadence
			// -scaled ramp every other riding mode gets via set_motor_ramp()
			// instead of the fixed rate below. That fixed rate
			// (CRUISE_DUTY_CYCLE_RAMP_UP_INVERSE_STEP) is tuned for genuine
			// Cruise mode, entered from the display's hidden menu only once
			// already near the target speed (gentle corrections) - at real SPORT
			// /TURBO speeds it's ~6-7x slower than set_motor_ramp()'s
			// PWM_DUTY_CYCLE_RAMP_UP_INVERSE_STEP_MIN (used above 20 km/h), so the
			// override was chronically unable to ramp up fast enough to hold
			// speed under load, unlike PAS TURBO at the same speed.
			set_motor_ramp();
		}
		else {
...
static void apply_speed_limit(void)
{
	if (ui8_wheel_speed_max > 0U) {
		// ui8_wheel_speed_max +/- 2, each clamped to stay inside uint8_t before
		// the multiply below - unclamped, a configured max of 254 or 255 wraps
		// (255+2 -> 1) turning the intended "no limit" setting into an
		// immediate near-zero cutoff (duty cycle killed at any real speed).
		// Only the near-255 direction matters in practice (the >0U check above
		// already excludes the near-0 underflow case for any wheel_speed_max
		// worth configuring), but both ends are clamped for symmetry.
		uint8_t ui8_speed_max_minus_2 = (ui8_wheel_speed_max > 2U) ? (uint8_t)(ui8_wheel_speed_max - 2U) : 0U;
		uint8_t ui8_speed_max_plus_2 = (ui8_wheel_speed_max < 253U) ? (uint8_t)(ui8_wheel_speed_max + 2U) : 255U;
		uint16_t speed_limit_low  = (uint16_t)(ui8_speed_max_minus_2 * (uint8_t)10U); // casting literal to uint8_t ensures usage of MUL X,A
		uint16_t speed_limit_high = (uint16_t)(ui8_speed_max_plus_2 * (uint8_t)10U);
...
#define TOFFSET_CYCLES 120 // 3sec (25ms*120)
    ...
    if (ui16_adc_pedal_torque > ui16_adc_pedal_torque_offset) {
		// calculate torque delta calibrated
		...
		// calculate torque delta estimated
		else if ((m_configuration_variables.ui8_torque_sensor_estimated)
		  && (!m_configuration_variables.ui8_torque_sensor_adv_enabled)) {
				ui16_adc_pedal_torque_delta = ((ui16_adc_pedal_torque - ui16_adc_pedal_torque_offset)
					* m_configuration_variables.ui8_pedal_torque_per_10_bit_ADC_step_est_x100)
					/ PEDAL_TORQUE_PER_10_BIT_ADC_STEP_BASE_X100;
		}
		else {
			ui16_adc_pedal_torque_delta = ui16_adc_pedal_torque - ui16_adc_pedal_torque_offset;
		}
...
#define CHECK_TORQUE_SENSOR_COUNTER_THRESHOLD          20 // 20 * 100ms = 2 seconds
    ...
    if (ui8_riding_torque_mode) {
		...
		if (ui8_check_torque_sensor_counter > CHECK_TORQUE_SENSOR_COUNTER_THRESHOLD) {
...
//#define MOTOR_BLOCKED_COUNTER_THRESHOLD 10
...
{
#if ENABLE_860C_LVGL_UART
	// CRC16 variable-length protocol (860C/850C): 3-state machine. Mirrors the
	// reference UART2_RX_IRQHandler exactly. Does NOT disable the RX interrupt
	// here - the ui8_received_package_flag guard below protects the buffer until
	// the main loop's communications_controller() clears the flag.
	if (UART2_GetFlagStatus(UART2_FLAG_RXNE) == SET) {
		UART2->SR &= (uint8_t)~(UART2_FLAG_RXNE); // this may be redundant

		if (ui8_received_package_flag == 0) { // only when package was previously processed
			...
			switch (ui8_state_machine) {
				...
				case 1:
					if (ui8_byte_received > (UART_RX_BUFFER_LEN - 2)) {
						ui8_state_machine = 0;
					}
					else {
...
#if ENABLE_860C_LVGL_UART
// Map this repo's single-value ui8_system_state (plus the 860C-specific NOT_INIT
// and FATAL bits) onto the display's error bitmask (see M_ERROR_* in main.h).
static uint8_t m_system_state_get(void)
{
	uint8_t m_state = ui8_m_system_state;

	if (ui8_battery_SOC_saved_flag) {
		m_state |= M_ERROR_FATAL; // undervoltage shutdown
	}

	switch (ui8_system_state) {
		case ERROR_TORQUE_SENSOR:       m_state |= M_ERROR_TORQUE_SENSOR; break;
		case ERROR_CADENCE_SENSOR:      m_state |= M_ERROR_CADENCE_SENSOR; break;
		case ERROR_MOTOR_BLOCKED:       m_state |= M_ERROR_MOTOR_BLOCKED; break;
		case ERROR_MOTOR_CHECK:         m_state |= M_ERROR_FATAL; break;
		case ERROR_THROTTLE:            m_state |= M_ERROR_THROTTLE; break;
		case ERROR_BATTERY_OVERCURRENT: m_state |= M_ERROR_BATTERY_OVERCURRENT; break;
		case ERROR_SPEED_SENSOR:        m_state |= M_ERROR_SPEED_SENSOR; break;
		default: break;
	}
	return m_state;
}

// Dispatcher, called from the main loop. Mirrors the reference implementation's
// communications_controller() exactly: CRC-check the received frame, keep the
// comms-failure safety counter, self-announce ALIVE before init, and dispatch by
// frame type.
static void communications_controller(void)
{
	...
	// comms-failure safety cutout: stop the motor if the display goes silent for
	// ~1500ms (30 misses at ~50ms per dispatcher tick). This is the mechanism
	// that stops the motor mid-ride if the display cable is disconnected.
	if (ui8_comm_error_counter > 30) {
		motor_disable_pwm();
		ui8_motor_enabled = 0;
		ui8_m_system_state |= M_ERROR_FATAL;
	}
...
// Builds the reply frame for a given frame type (or the unprompted ALIVE) and
// sends it. Mirrors the reference communications_process_packages() exactly,
// including the little-endian CRC16 tail and the receive-flag handshake.
static void communications_process_packages(uint8_t ui8_frame_type)
{
	uint8_t ui8_temp;
	uint16_t ui16_temp;
	uint8_t ui8_len = 3; // 1 type byte + 2 CRC bytes
	uint8_t ui8_i;

	// start up byte (motor -> display)
	ui8_tx_buffer[0] = TX_STX; // 0x43
	...
	switch (ui8_frame_type) {
	  ...
	  case COMM_FRAME_TYPE_PERIODIC:
		// display sends periodic command after motor init ok; reset status so the
		...
		if (ui8_target_battery_max_power_div25 != ui8_target_battery_max_power_div25_temp) {
			...
			uint32_t ui32_battery_current_max_x100 = ((uint32_t) ui8_target_battery_max_power_div25 * 25000UL)
				/ ui16_battery_voltage_filtered_x10;
			ui8_adc_battery_current_max_temp_2 = (uint8_t)(ui32_battery_current_max_x100 / BATTERY_CURRENT_PER_10_BIT_ADC_STEP_X100);
		...
		ui8_tx_buffer[3] = (uint8_t)(ui16_adc_voltage_filtered & 0xff);
		ui8_tx_buffer[4] = ((uint8_t)(ui16_adc_voltage_filtered >> 4)) & 0x30;

		// battery current x5 (this repo keeps x10; shift once)
		ui8_tx_buffer[5] = (uint8_t)(ui8_battery_current_filtered_x10 >> 1);

		// wheel speed x10 (10 bits, capped by the display at 999)
		ui8_tx_buffer[6] = (uint8_t)(ui16_wheel_speed_x10 & 0xff);
		ui8_tx_buffer[7] = ((uint8_t)(ui16_wheel_speed_x10 >> 8)) & 0x07;

		// brake state + hall sensors + flags
		ui8_tx_buffer[8] = (ui8_brake_state & 1);
		ui8_tx_buffer[8] |= (uint8_t)(ui8_hall_state_irq & 7) << 1;
		ui8_tx_buffer[8] |= 0; // bit4 speed-limit-high: not tracked in this repo
		ui8_tx_buffer[8] |= ((uint8_t)((ui16_adc_voltage_filtered < ui16_adc_voltage_cut_off) ? 1U : 0U) << 5); // voltage cut-off
		ui8_tx_buffer[8] |= 0; // bit6 voltage-shutdown: not tracked in this repo
		ui8_tx_buffer[8] |= (uint8_t)((PWM_FREQ - 18) & 1) << 7; // pwm frequency flag

		// throttle ADC
		ui8_tx_buffer[9] = (uint8_t)(ui16_adc_throttle >> 2);
		// optional ADC value (temperature or mapped throttle)
		switch (ui8_optional_ADC_function) {
			...
			case THROTTLE_CONTROL:
				ui8_tx_buffer[10] = ui8_throttle_adc_map;
				break;
			default:
				ui8_tx_buffer[10] = 0;
				break;
		...
		ui8_tx_buffer[7] |= (uint8_t)((ui16_adc_torque & 0x300) >> 2); // shared with wheel speed bits
		...
		// FOC angle (4 bits) + field weakening angle (2 bits)
		ui8_tx_buffer[18] = ui8_g_foc_angle;
		ui8_tx_buffer[18] |= (uint8_t)(ui8_fw_hall_counter_offset << 4);

		// error states (bitmask)
		ui8_tx_buffer[19] = m_system_state_get();

		// motor current x5
		ui8_tx_buffer[20] = (uint8_t)(((uint16_t)ui8_adc_motor_phase_current * BATTERY_CURRENT_PER_10_BIT_ADC_STEP_X100) / 20);

		// wheel speed sensor tick counter (this repo keeps 16 bits; upper byte 0)
		ui8_tx_buffer[21] = (uint8_t)(ui16_wheel_speed_sensor_ticks & 0xff);
		ui8_tx_buffer[22] = (uint8_t)(ui16_wheel_speed_sensor_ticks >> 8);
		ui8_tx_buffer[23] = 0;
	  ...
	  case COMM_FRAME_TYPE_CONFIGURATIONS:
		// disable the motor while configurations change; it is re-enabled by
		// ebike_control_motor() once the init delay elapses (MOTOR_INIT_OK).
		motor_disable_pwm();
		ui8_motor_enabled = 0;
		ui8_m_system_state |= M_ERROR_NOT_INIT;
	...
	// prepare CRC16 of the package (little-endian)
	ui16_crc_tx = 0xffff;
	for (ui8_i = 0; ui8_i < ui8_len; ui8_i++) {
	...
	// send the full buffer (UART_TX_BUFFER_LEN == 29 bytes in 860C mode; the
	// display reads ui8_tx_buffer[1] and ignores trailing bytes)
...
static void uart_receive_package(void)
{
	...
	if (ui8_received_package_flag) {
		...
		if (ui8_rx_check_code == ui8_rx_buffer[RX_CHECK_CODE]) {
			...
			// snapshot the assist level while the walk assist button is not held, before it
			// is potentially overwritten below with a level the display reports while the
			// button is held (see ui8_assist_level_before_walk_button declaration)
			if (!ui8_walk_assist_button_pressed) {
			...
			// also leaves ui8_riding_mode == CRUISE_MODE while held, which
			// would otherwise match here first on every poll after the first
			// and permanently shadow the override's own branch below (the
			// only place that restores ui8_riding_mode_temp on release) -
			// latching the bike into CRUISE_MODE with no way back except a
			// display reboot. See UNIVERSAL_FIRMWARE_PLAN.md's matching entry.
			else if ((m_configuration_variables.ui8_riding_mode == CRUISE_MODE)
			  &&(!ui8_cruise_override_flag)) {
				if ((ui8_walk_assist_button_pressed)&&(ui8_startup_flag)) {
			...
			// startup assist mode and walk assist mode ******************
			  ...
			  &&((!ui8_torque_sensor_calibration_enabled)
				...
				// cruise control override: holding the walk-assist button at an
				// overridden level (any of ECO/TOUR/SPORT/TURBO, individually
				// selectable - see CRUISE_OVERRIDE_ACTIVE_LEVEL) temporarily
				// borrows apply_cruise()'s real PID speed-hold instead of walk
				// assist's own ramp below - see main.h's
				// CRUISE_OVERRIDE_ACTIVE_LEVEL and UNIVERSAL_FIRMWARE_PLAN.md's
				// matching entry. Compiled out entirely when no override is
				// configured (CRUISE_OVERRIDE_ACTIVE_LEVEL folds to a constant
				// false) - #if'd out rather than left as dead runtime code
				// because SDCC's optimizer proves the branch unreachable and
				...
				if ((ui8_walk_assist_button_pressed)&&(ui8_startup_flag)&&(!ui8_startup_assist_flag)
				  &&(CRUISE_OVERRIDE_ACTIVE_LEVEL(ui8_assist_level_before_walk_button))) {
					if (!ui8_cruise_override_flag) {
						...
						// speed target on its very first call for this activation.
						// Organic cruise use (selected as a persistent riding mode) always
						// gets a few disengaged polls first (not yet pedaling/up to speed),
						// which is what normally sets this flag - see its disengage-branch
						// assignment below. The override can engage on its very first poll
						// (button + threshold already satisfied), skipping that warm-up
						// entirely, so without this the PID init block never runs and
						// ui16_wheel_speed_target_x10 is left at 0 - motor stays silent.
						ui8_cruise_PID_initialize = 1;
					}
					ui8_cruise_button_flag = 1;
				}
				...
				// reset walk assist flag
				ui8_walk_assist_flag = 0;
				// reset cruise override flag (mirrors walk assist's own reset just
				// above - if the button/level combo stops qualifying entirely, e.g.
				// assist level goes to OFF, make sure a still-active override
				// doesn't leave riding mode stuck on CRUISE_MODE)
				if (ui8_cruise_override_flag) {
					m_configuration_variables.ui8_riding_mode = ui8_riding_mode_temp;
					ui8_cruise_override_flag = 0;
				}
			...
			// same decremented-level display quirk as the WALK_ASSIST_MODE case
			// below applies while the cruise override is holding riding_mode on
			// CRUISE_MODE via the walk-assist button - use the pre-button level
			// there too, or a display that reports the lowest level while held
			// would index cruiseSpeed at level 0 (no value defined -> 0), which
			// trips the ui8_riding_mode_parameter == 0U safety cutoff above and
			// zeroes the motor output entirely instead of cruising
			if ((m_configuration_variables.ui8_riding_mode == WALK_ASSIST_MODE)
			  ||((m_configuration_variables.ui8_riding_mode == CRUISE_MODE)&&(ui8_cruise_override_flag))) {
				// use the level from before the walk assist button was pressed: some displays
				// report a decremented assist level while the button (shared with level-down)
				// is held, which would otherwise always select the lowest walk speed
...
// Battery sag indicator (E11) - purely informational, doesn't disable the motor:
// motor.c's undervoltage ramp-down (what this reports on) runs unconditionally
// regardless of ENABLE_BATTERY_SAG_INDICATOR, disabling it here only stops the display
// from showing it. Guarded on NO_FAULT so this never overrides a real fault already set
// by the caller before this runs (overvoltage/undervoltage-shutdown) - the unconditional
// temperature/motor-blocked checks that run after this can still override it in turn,
// same as they already override each other. A small standalone function (not inlined
// into uart_send_package()) so it's directly testable without needing that much larger
// function's display-type/ready-flag preconditions - same reasoning as
// filter_undervoltage_check_voltage() in motor.c.
static void check_battery_sag_indicator(void)
{
#if ENABLE_BATTERY_SAG_INDICATOR
	if ((ui8_display_fault_code == NO_FAULT) && (ui16_adc_voltage_filtered < ui16_adc_voltage_cut_off)) {
		ui8_display_fault_code = ERROR_BATTERY_SAG;
	}
#endif
}
...
static void check_battery_soc(void)
{
	...
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_SOC_RESET_X10) { ui8_battery_state_of_charge = 6; }	// 4 bars -> SOC reset
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_FULL_X10) { ui8_battery_state_of_charge = 5; }	// 4 bars -> full
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_3_X10) { ui8_battery_state_of_charge = 4; }	// 3 bars
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_2_X10) { ui8_battery_state_of_charge = 3; }	// 2 bars
	...
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_5_X10) { ui8_battery_state_of_charge = 6; }	// 5 bars
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_4_X10) { ui8_battery_state_of_charge = 5; }	// 4 bars
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_3_X10) { ui8_battery_state_of_charge = 4; }	// 3 bars
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_2_X10) { ui8_battery_state_of_charge = 3; }	// 2 bars
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_1_X10) { ui8_battery_state_of_charge = 2; }	// 1 bar
	else if (ui16_battery_voltage_calibrated_and_filtered_x10 > BATTERY_SOC_VOLTS_EMPTY_X10) { ui8_battery_state_of_charge = 1; }	// blink -> empty
